...
export function WorkEntry({ project, compact = false, category }: { project: Project; compact?: boolean; category?: string }) {
  ...
  return <article className={styles.card} id={project.slug}>
    ...
    <div className={styles.body}>
      ...
      {project.gallery?.length ? <details className={styles.evidence}><summary>View original project images ({project.gallery.length})</summary><div>{project.gallery.map(image => <a key={image.src} href={image.src} target="_blank" rel="noreferrer"><img src={image.src} alt={image.alt} loading="lazy" /></a>)}</div></details> : null}
