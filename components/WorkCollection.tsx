...
export function WorkEntry({ project, compact = false, category }: { project: Project; compact?: boolean; category?: string }) {
  ...
  return <article className={styles.card} id={project.slug}>
    ...
    </div> : compact ? <Link className={styles.visual} href={destination} aria-label={`Explore ${project.title}`}><img src={previewImage} alt={previewAlt} loading="lazy" /></Link> : <a className={styles.visual} href={project.image} target="_blank" rel="noreferrer" aria-label={`View ${project.title} image`}><img src={project.image} alt={`${project.title} project preview`} loading="lazy" /></a>}
    <div className={styles.body}>
      ...
      {project.gallery?.length ? <details className={styles.evidence}><summary>{project.galleryLabel ?? `View original project images (${project.gallery.length})`}</summary><div>{project.gallery.map(image => <a key={image.src} href={image.src} target="_blank" rel="noreferrer"><img src={image.src} alt={image.alt} loading="lazy" /></a>)}</div></details> : null}
      {project.uiGallery?.length ? <details className={styles.mobileEvidence}><summary>Mobile app screens ({mobileScreens[project.slug]?.length ?? project.uiGallery.length})</summary>
        ...
        <details className={`${styles.evidence} ${styles.interfaces}`}><summary>View original UI images</summary><div>{project.uiGallery.map(image => <a key={image.src} href={image.src} target="_blank" rel="noreferrer"><img src={image.src} alt={image.alt} loading="lazy" /></a>)}</div></details>
      </details> : null}
      </>}
    </div>
  </article>;
