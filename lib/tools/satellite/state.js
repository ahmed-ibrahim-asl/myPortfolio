...
export function encodeProblem(slug, values, mode = "course") {
  const payload = JSON.stringify({ slug, values, mode });
  return `?v=1&p=${encodeURIComponent(payload)}`;
}
export function decodeProblem(query, allowedFields = []) {
  try {
    const params = new URLSearchParams(query);
    if (params.get("v") !== "1" || (params.get("p")?.length ?? 0) > 12000) return null;
    ...
    return { slug: p.slug, values, mode: p.mode === "engineering" ? "engineering" : "course" };
  } catch {
    return null;
  }
}
