...
test(
  "theme choice persists and tool categories stay focused in both themes",
  { timeout: 90_000 },
  async (t) => {
    if (!browserPath) {
    ...
    try {
      ...
      const firstVisit = await page.evaluate(() => ({
        ...
        languageBetweenToggleAndContact: Boolean(
          document.querySelector(".theme-toggle")?.nextElementSibling?.matches(".language-switch") &&
          document.querySelector(".language-switch")?.nextElementSibling?.matches(".header-contact")
        )
      }));
      assert.deepEqual(firstVisit, {
        ...
        categoryCount: 8,
        languageBetweenToggleAndContact: true
      });
      ...
      await page.click(".theme-toggle");
      assert.deepEqual(
        await page.$eval(".theme-toggle", (button) => ({
          labels: [...button.querySelectorAll(".theme-toggle-action > span")].map((node) =>
      ...
      assert.deepEqual(
        await page.$$eval(".asl-tool-category-catalog section > h2", (nodes) =>
